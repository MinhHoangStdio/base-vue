...
const handleChange = (e,value)=>{console.log({from:e.from.getAttribute("data-id")});console.log({to:e.to.getAttribute("data-id")}); console.log(value)}
</script>

<template>
  <div class="grid grid-cols-3 gap-6">
    <div v-for="lane in lanes" :key="lane.name" class="border border-gray-300 rounded-md bg-gray-50">
      <div class="
					bg-white
        ...
        <div class="flex items-center space-x-4">
          <button v-if="lane.name == 'Done'" class="text-blue-500 hover:text-blue-700 font-semibold">
            Clear all
          </button>
      ...
      <div class="p-4 h-full">
        <draggable :data-id="lane.name" @end="(event)=>handleChange(event,lanes)" class="min-h-full" :list="lane.tickets" group="tickets" itemKey="name" v-bind="dragOptions">
